...
(defun strongly-connected-components (g)
  (dfs g)
  (let ((h (make-hash-table))
        (gt))
    (dolist (u *list* (nreverse gt))
      (push (setf (gethash u h) (make-vertex)) gt))
    (dolist (u *list* (dfs gt))
      (dolist (v (vertex-alist u))
        (push (gethash u h) (vertex-alist (gethash v h)))))))
